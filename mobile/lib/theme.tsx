...
export function statusTone(theme: ThemePalette): Record<AgentStatusKey, StatusTone> {
  ...
  return {
    idle: { label: '空闲', color: dark ? '#5A5A58' : '#B8B8B4', textColor: theme.textFaint },
    thinking: { label: '处理中', color: thinking, textColor: thinkingText },
    complete: { label: '已完成', color: theme.online, textColor: theme.online },
    waiting: { label: '等待输入', color: waiting, textColor: waitingText },
    error: { label: '出错', color: theme.danger, textColor: theme.danger },
  };
}
