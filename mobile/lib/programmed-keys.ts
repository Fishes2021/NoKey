import { normalizeShortcut, type KeyboardShortcut } from './keyboard-shortcuts.mjs';

export const PROGRAMMED_KEY_COUNT = 10;
export const programmedKeysStorageKey = (keyId: string) => `voicedeck.keys.${keyId}`;
...
export type ProgrammedKeyAction =
  | ({ type: 'shortcut' } & KeyboardShortcut)
  | {
      type: 'command';
...
export function defaultProgrammedKeys(): (ProgrammedKey | null)[] {
  const keys: (ProgrammedKey | null)[] = ([
    ['Escape', 'Esc'], ['Tab', 'Tab'], ['Backspace', '删除'],
    ['Enter', '发送'], ['Enter', '换行'], ['Z', '重做'],
...
export function programmedActionId(programmed: ProgrammedKey | null | undefined) {
  ...
  if (programmed.action.type === 'shortcut') return 'voicedeck.shortcut';
...
export function legacyActionIdForProgrammedKey(
  ...
  if (programmed.action.type !== 'command') return null;
...
function parseAction(value: unknown): ProgrammedKeyAction | null {
  ...
  if (value.type === 'shortcut') {
    try { return { type: 'shortcut', ...normalizeShortcut(value) }; }
    catch { return null; }
  }
  if (
    value.type === 'command' &&
...
export function parseProgrammedKeys(raw: string | null | undefined) {
  ...
  try {
    ...
    if (!Array.isArray(parsed) || ![6, PROGRAMMED_KEY_COUNT].includes(parsed.length)) {
      return defaultProgrammedKeys();
    }
    return [...parsed, ...Array(PROGRAMMED_KEY_COUNT - parsed.length).fill(null)].map((entry, slotIndex) => {
      ...
      return {
        ...
        // Only legacy caps without an assignment inherit a default. Never turn
        // a cleared or corrupt action into a different executable command.
        action: 'action' in entry ? parseAction(entry.action) : defaultActionForKeycap(entry.keycapId),
        ...(typeof entry.label === 'string' && entry.label.trim() && entry.label.trim().length <= 24
...
// Keep the legacy decoder for stored-data compatibility; the product only runs
// system shortcuts. Preserve explicitly empty slots and existing generic keys.
