...
export function rotationDelta(previous: number, next: number) {
  'worklet';
...
export function ReasoningDial({
  ...
  const handlers = useRef({ onPreview, onCommit, onStep, onPress, onLongPress });
  handlers.current = { onPreview, onCommit, onStep, onPress, onLongPress };
  ...
  const dialSize = useSharedValue({ width: 1, height: 1 });
  ...
  const preview = useCallback((nextIndex: number) => {
    ...
    handlers.current.onPreview(bounded);
    return bounded;
  }, [maxIndex]);
  ...
  const emitEncoderSteps = useCallback((crossed: number) => {
    ...
    encoderFlushTimer.current = setTimeout(() => {
      ...
      if (pending) handlers.current.onStep(pending > 0 ? 1 : -1, Math.abs(pending));
    }, ENCODER_FLUSH_MS);
  }, []);
  ...
  const commitDragged = useCallback((settledIndex: number) => {
    ...
    panCommitTimer.current = setTimeout(() => {
      ...
      handlers.current.onCommit(settledIndex);
    }, 90);
  }, []);
  ...
  const dialGesture = useMemo(() => {
    ...
    const encoderPan = Gesture.Pan()
      ...
      .onBegin((gesture) => {
        'worklet';
        dragging.value = true;
        fingerAngle.value = Math.atan2(gesture.y - dialSize.value.height / 2, gesture.x - dialSize.value.width / 2) * 180 / Math.PI;
      ...
      .onUpdate((gesture) => {
        'worklet';
        const x = gesture.x - dialSize.value.width / 2;
        const y = gesture.y - dialSize.value.height / 2;
        const nextAngle = Math.atan2(y, x) * 180 / Math.PI;
        const delta = rotationDelta(fingerAngle.value, nextAngle);
        fingerAngle.value = nextAngle;
        // Ignore the centre, where tiny finger motion has an unstable angle.
        if (Math.hypot(x, y) < Math.min(dialSize.value.width, dialSize.value.height) * 0.18) return;
        angle.value += delta;
        const raw = dragPosition.value + delta / DEGREES_PER_DETENT;
    ...
    const tap = Gesture.Tap()
      ...
      .onEnd((_event, success) => {
        ...
        handlers.current.onCommit(bounded);
      });
    ...
    const longPress = Gesture.LongPress()
      ...
      .maxDistance(12)
      .runOnJS(true)
      .onStart(() => handlers.current.onLongPress());
  ...
  return (
    <GestureDetector gesture={dialGesture}>
      <View
        onLayout={event => { dialSize.value = { width: event.nativeEvent.layout.width, height: event.nativeEvent.layout.height }; }}
        collapsable={false}
        ...
        onAccessibilityAction={(event) => {
          ...
          if (mode === 'reasoning') {
            ...
            handlers.current.onStep(delta, 1);
          }
        }}
        style={styles.module}>
        <View style={styles.shadow} />
        <LinearGradient
          colors={['#FFFFFF', '#F5F3EE', '#E9E6DF', '#D4D1C8', '#ECE9E2']}
          ...
          <Animated.View style={[styles.dialCapWrapper, capStyle]}>
            <LinearGradient
              colors={['#FCFBF8', '#F7F5F0', '#F3F1EB', '#EDEAE3', '#E8E5DC']}
              locations={[0, 0.25, 0.49, 0.74, 1]}
              start={{ x: 0.04, y: 0.08 }}
              end={{ x: 0.94, y: 0.92 }}
              style={styles.dialCap}>
              <LinearGradient
                colors={['#41463F', '#41463F', '#41463F', '#41463F']}
...
const styles = StyleSheet.create({
  ...
  groove: {
    position: 'absolute', width: 4, height: '16%', top: '12%', left: '48%', borderRadius: 2,
  },
  capGlint: {
    position: 'absolute',
    left: '18%',
