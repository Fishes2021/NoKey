...
export function usePhoneMicrophone(bridgeUrl: string, token: string, encryption: E2EEKeyMaterial | null) {
  ...
  const stop = useCallback(async () => {
    ...
    try {
      await previous?.stop();
      if (generation.current === stoppedGeneration) setMessage('手机麦克风已停止');
    } catch (error) {
  ...
  const toggle = useCallback(async () => {
    ...
    try {
      ...
      current = new VoiceSender({
        ...
        getIceConfig: async () => validateIceConfig(await bridgeRequest(bridgeUrl, token, '/api/voice/config', { method: 'POST', body: {} }, encryption)),
        createPeer: configuration => new rtc.RTCPeerConnection(configuration),
        getStream: guardedCapture.getStream,
        request: (path, body) => bridgeRequest(bridgeUrl, token, path, { method: 'POST', body }, encryption),
