...
test('the lower panel is a chat-styled one-way remote composer', () => {
  ...
  assert.match(controllerSource, /发送到 Codex/);
  assert.match(controllerSource, /结果显示在 Mac/);
  assert.match(controllerSource, /输入要发送给 Codex 的内容/);
  assert.match(controllerSource, /连接 Mac 后输入内容/);
