...
**Mac 下载及状态见 [Releases](https://github.com/Fishes2021/NoKey/releases)。** iPhone 1.0.0 正在准备 App Store 上架，尚未公开发布。局域网已在开发设备使用；自用中继曾完成跨网连接，当前公共中继不开放。长时间语音与更多设备兼容性仍需实际使用验证。
...
- 连续光标旋钮、六个快捷键和四方向摇杆。
- Mac 启动时选择虚拟麦克风，停止讲话时保持；关闭窗口隐藏到菜单栏，主动退出才恢复原音源。
...
5. 快捷槽可按界面入口配置，语音开关在 Mac 设置中自定义；摇杆默认方向键，长按连续移动，松手停止。不要向不信任的设备提供配对二维码。

## 国内中继

服务器运行 Node 加密信令转发、coturn 媒体中继及 HTTPS 反向代理，不运行语音识别。部署准备工具生成独立配置和随机密钥，不自动启动服务。

见 [中继说明](relay/README.md)和[部署手册](relay/deploy/RUNBOOK.md)。目前没有公共中继地址；部署后还需检验蜂窝到 Mac、持续讲话、切网和 TURN-TLS。已实现月付／年付激活码、单 Mac 绑定、续期和到期断开，不做流量额度。见[中继授权说明](relay/deploy/SUBSCRIPTIONS.md)。自动收款、账号系统及运营告警未实现，线上授权部署尚待验收。
