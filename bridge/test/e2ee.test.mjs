...
test('encrypted clients persist locally and reject replayed session messages', async () => {
  ...
  try {
    ...
    await registry.removeClient(material.keyId);
    assert.throws(() => registry.sealResponse(context, {}));
    assert.throws(() => registry.sealEvent(context, {}));
    await assert.rejects(new E2EEClientRegistry({ stateDir }).materialFor(material.keyId));
  } finally {
