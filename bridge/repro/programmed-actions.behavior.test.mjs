  ...
  const codex = {
    ...
    async state() {
      ...
      return {
        ...
        selected,
        pendingApproval: { requestId: 'approval-1', threadId: 'thread-1' },
      };
    },
    ...
    async resolveApproval(decision, approval) {
      assert.deepEqual(approval, { requestId: 'approval-1', threadId: 'thread-1' });
...
test('every available programmable key reaches a supported action path', async () => {
  for (const action of CODEX_PROGRAMMABLE_ACTIONS) {
    ...
    await assert.doesNotReject(
      executeProgrammedAction({
        commandId: action.id,
        approval: { requestId: 'approval-1', threadId: 'thread-1' },
