...
export class CodexAppServer {
  ...
  constructor({ lazy = false } = {}) {
    ...
    if (!lazy) {
      ...
      this.#readyPromise.catch(() => {}); // Report failures to callers awaiting ready().
    }
  }
  ...
  async #start() {
    ...
    if (this.#closed) throw new Error('Codex client is closed.');
    const shared = await acquireSharedServer(codexBin);
    if (this.#closed) {
      releaseSharedServer(shared);
      throw new Error('Codex client is closed.');
    }
    this.#sharedServer = shared;
    try {
      const socket = await openWebSocket(shared.url);
      if (this.#closed) {
        socket.close();
        throw new Error('Codex client is closed.');
      }
  ...
  async resolveApproval(decision, approval) {
    if (!['approve', 'decline'].includes(decision)) {
      throw Object.assign(new Error('Invalid approval decision.'), { statusCode: 400 });
    }
    const selected = typeof approval?.requestId === 'string' ? this.#approvals.get(approval.requestId) : null;
    if (!selected || selected.threadId !== approval.threadId || selected.threadId !== this.#selectedThreadId) {
      throw Object.assign(new Error('审批已变化，请刷新并确认当前请求。'), { statusCode: 409 });
    }
