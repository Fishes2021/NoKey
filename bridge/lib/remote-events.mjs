...
export function attachRemoteEvents({
  ...
  const unsubscribeRevoked = e2eeClients?.subscribeRevoked(keyId => {
    for (const socket of sockets) if (socket.microdexE2EEContext?.material.keyId === keyId) {
  ...
  function sendPayload(socket, payload) {
    ...
    if (socket.microdexE2EEContext && e2eeClients) {
      ...
      })); } catch { socket.microdexAuthenticated = false; socket.close(4401, 'Authorization expired'); }
      return;
    }
