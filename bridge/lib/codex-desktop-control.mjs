...
const bundled = Boolean(process.resourcesPath && bridgeDir === path.join(process.resourcesPath, 'app', 'bridge'));
const executablePath = bundled ? path.join(bridgeDir, 'native', '语音快捷键盘操作助手') : path.join(installDir, 'MicrodexDesktop');
...
async function needsBuild() {
  // A distributed app must never fall back to compiling on the user’s Mac.
  if (bundled) { await stat(executablePath); return false; }
  try {
