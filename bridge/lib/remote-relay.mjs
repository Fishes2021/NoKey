...
export function createRemoteRelay({
  ...
  const unsubscribeRevoked = e2eeClients?.subscribeRevoked(keyId => {
    for (const [id, context] of encryptedPhones) if (context.material.keyId === keyId) encryptedPhones.delete(id);
  });
  let current = {
  ...
  const openLocalEvents = () => {
    ...
    socket.once('open', () => {
      socket.send(JSON.stringify({ type: 'auth', token: localToken }));
    });
      ...
      try {
        ...
        if (!lastState) {
          const response = await fetchImpl(`http://127.0.0.1:${port}/api/remote/state`, {
            headers: { Accept: 'application/json', 'X-Microdex-Token': localToken },
          });
  ...
  const answerEncryptedRelayRequest = async (message, safePath) => {
    ...
    try {
      ...
      const handled = await handleAuthenticatedRequest?.(context);
      if (handled) {
        const envelope = e2eeClients.sealResponse(context, handled);
  ...
  const scheduleReconnect = () => {
    ...
    reconnectAttempt += 1;
    const delay = delayForAttempt(reconnectAttempt);
    reconnectTimer = setTimeout(() => {
      reconnectTimer = null;
      void connect();
    }, delay);
    publish({ retryAt: Date.now() + delay });
  };

  const connect = async () => {
    if (closed || !enabled || relaySocket) return;
    publish({ status: 'connecting', ready: false, retryAt: null });
    try {
      ...
      socket.once('open', () => {
        reconnectAttempt = 0;
        publish({ status: 'ready', ready: true, url: publicUrl, error: null, retryAt: null });
        openLocalEvents();
        let alive = true;
      ...
      socket.once('close', () => {
        ...
        publish({
          ...
          ready: false,
          url: publicUrl,
          error: current.error || 'The stable relay disconnected and is reconnecting.',
        });
        scheduleReconnect();
      });
    } catch (error) {
  ...
  return {
    ...
    close() {
      ...
      publish({ status: 'closed', ready: false, error: null, retryAt: null });
      listeners.clear();
    },
  };
}
